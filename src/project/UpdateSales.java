...
public class UpdateSales implements Initializable {
    ...
    private final ObservableList partyname = FXCollections.observableArrayList();
    private final ObservableList invoiceno = FXCollections.observableArrayList();// = FXCollections.observableArrayList("Jethalal books ","Ujjwal book Depot" ,"Socity book Center");
    ...
    public void init_1() throws SQLException {
        ...
        sinvoiceno.setItems(invoiceno);
        sinvoiceno.setPromptText("Select Invoice");
        str = "select * from saletable";
        System.out.println("shailesh");
        ps = con.prepareStatement(str);
        rs = ps.executeQuery();
        
        while (rs.next()) {
            invoiceno.add(rs.getString("invoice"));
        }
        String party = (String) sinvoiceno.getValue();
        str1 = "select * from saletable where invoice=?";
        System.out.println("shailesh11");
        ps1 = con.prepareStatement(str1);
        ps1.setString(1, party);
        rs1 = ps1.executeQuery();
        System.out.println("shailesh11111");
        
        if (rs1.next()) {
            System.out.println("shailesh");
            //smode.setText(rs1.getString("partytype"));
            smode.setText("Credit");
            slocation.setText(rs1.getString("location"));
            System.out.println("shailesh");
            staxcode.setPromptText(rs1.getString("tax"));
...
//            pst.setString(2, s2);
            ...
            Project.status.setText("Data added to saletable database !");
